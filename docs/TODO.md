...
- [ ] 起動直後のマップにGPSで大まかな現在地を表示する（初回とボタン押下時のみ取得。追跡はしない）
- [ ] 地上ルートが最適な場合の提案とGoogle Mapsへの誘導（日差し回避などの理由がなければ地上を提案）
- [ ] 案内中ビューの磨き込み（三人称視点の追従表示。3Dは色付きボックスの軽量表現を維持し、注目すべき店名を視認サイズで表示）
- [ ] 音声案内（次の動作カードの読み上げ）
- [ ] B2フロアの実データ割当（阪急三番街南館/北館B2など。現状は全地下ノードB1既定）
- [ ] タブバーに「お気に入り」タブを追加
...
- [x] モックアップUI再現（2026-07-11）: フロア概念B2/B1/1F・フロア移動ステップ、エリアゾーン・出口バッジ・設備チップ・凡例、デュアル入力+タブバーのアプリシェル、クイックチップ+推定位置カード、次の動作カード+到着まで表示+階段回避トグル
...
B2フロアの実データ割当（阪急三番街）とマップUIの磨き込み。並行して報告フォームの回答確認の運用開始
